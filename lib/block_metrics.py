...
def block_influence(
    ...
    input_hidden_state = input_hidden_state.to(torch.float32)
    output_hidden_state = output_hidden_state.to(torch.float32)


    if metrics == 'mse':
        return torch.mean((input_hidden_state - output_hidden_state) ** 2, dim=-1)


    if metrics == 'mae':
        return torch.mean(torch.abs(input_hidden_state - output_hidden_state), dim=-1)
